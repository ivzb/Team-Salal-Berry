<?php
include 'functions.php';
$title = "Инициативи | GreenUni ..";
$description = "инициативи";
$keywords = "инициативи";

my_header($title, $description, $keywords);
?>
      <section>
         <div class="container">
             <div class="row long">
                 <div class="col-md-6">
                     <h2>Инициативи</h2>
                     <p class="indent">
                        
                 </div>

                 <div class="col-md-6">
                     <img src="http://greenuni.cloudvps.bg/images/green-initiatives.jpg" class="right" alt="" />
                 </div>
             </div>

            <div class="row about">
               <div class="col-md-4">
                  <h2>Да рециклираме старите машини</h2>
                  <p>
                       <img class="fancybox" class="fancybox" src="images/eco.jpg" alt=""  />
                  </p>
               </div>
               <div class="col-md-4">
                  <h2>Да спасим клавиатурите от завършилите наши студенти</h2>
                   <p>
                       <img class="fancybox" src="http://greenuni.cloudvps.bg/images/keyboard.jpg" alt="" />
                   </p>
               </div>
               <div class="col-md-4">
                  <h2>Изнеси се от общежитие - спаси хлебарка</h2>
                   <p>
                       <img class="fancybox" src="http://greenuni.cloudvps.bg/images/bug.jpg" alt="" />
                   </p>
               </div>
            </div>

             <div class="row">
                 <div class="col-md-4">
                     <h2>Да бъдем екип</h2>
                     <p>
                         <img class="fancybox" src="http://greenuni.cloudvps.bg/images/teamwork.jpg" alt="" />
                     </p>
                 </div>
                 <div class="col-md-4">
                     <h2>Как да спечелим сто хиляди лева за един ден</h2>
                     <p>
                        <p>- Здравейте! Добре дошли на семинара "Как да спечелим сто хиляди лева за един ден"!</p>
                        <p>Ръкопляскания в залата</p>
                        <p>- А сега ми кажете, колко платихте за участие в семинара?</p>
                        <p>- 100 лв.</p>
                        <p>- И един последен въпрос. Колко човека има в залата?</p>
                        <p>- 1000.</p>
                        <p>- Благодаря! Семинарът завърши.</p>
                     </p>
                 </div>
                 <div class="col-md-4">
                     <h2>Да изчистим офиса за 1 ден</h2>
                     <p>
                         <img class="fancybox" src="http://greenuni.cloudvps.bg/images/harmony.jpg" alt=""  />
                        <img class="fancybox" src="http://greenuni.cloudvps.bg/images/harmony2.jpg" alt=""  />
                     </p>
                 </div>
             </div>

             <hr>
         </div>
      </section>
<?php 
my_foother();
?>      